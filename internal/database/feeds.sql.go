...
const getNextFeedTooFetch = `-- name: GetNextFeedTooFetch :one
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds
ORDER BY last_fetched_at ASC NULLS FIRST
LIMIT 1
`

func (q *Queries) GetNextFeedTooFetch(ctx context.Context) (Feed, error) {
	row := q.db.QueryRowContext(ctx, getNextFeedTooFetch)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
